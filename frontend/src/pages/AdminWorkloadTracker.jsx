...
const AdminWorkloadTracker = () => {
    ...
    try {
      ...
      setWorkloads(prev => prev.map(w => {
        if (w.id === docId) {
          ...
          newBreakdown[taskIndex] = { ...newBreakdown[taskIndex], isCompleted: newStatus };
  ...
  const handleSendEmail = async (studentId, week) => {
    ...
    try {
      ...
      loadData(); 
    } catch (error) { showToast(error.response?.data?.error || "Failed to send email.", "error"); } 
    finally { setProcessingId(null); }
  };
  ...
  const handleBulkEmail = async () => {
    ...
    try {
      const promises = pendingActions.map(action => triggerManualWarningEmail(action.studentId, action.week));
      await Promise.all(promises);
      showToast(`Successfully sent ${promises.length} emails!`);
      loadData();
    } catch (error) { showToast("Some emails failed. Check logs.", "error"); } 
  ...
  const pendingActions = workloads.map(w => ({ ...w, btnConfig: getEmailButtonConfig(w) }))
                                  .filter(w => !w.btnConfig.disabled);

  let filteredWorkloadsByWeek = workloads;
  if (weekFilter !== "All") filteredWorkloadsByWeek = workloads.filter(w => w.week.toString() === weekFilter);

  const groupedWorkloads = filteredWorkloadsByWeek.reduce((acc, curr) => {
    if (!acc[curr.studentId]) acc[curr.studentId] = { name: curr.studentName || "Unknown", workloads: [] };
  ...
  const filteredStudentIds = Object.keys(groupedWorkloads).filter(id => {
    ...
    const matchName = groupedWorkloads[id].name.toLowerCase().includes(searchTerm.toLowerCase());
    return matchId || matchName;
  });

  const uniqueWeeks = [...new Set(workloads.map(w => w.week))].sort((a, b) => a - b);

  if (loading) return <div className="flex justify-center items-center h-screen bg-slate-50"><Loader className="w-12 h-12 animate-spin text-indigo-600" /></div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 sm:p-10 font-sans text-gray-800 relative">
      <div className={`fixed bottom-6 right-6 z-[100] transition-all duration-500 transform ${toast.show ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'}`}>
        <div className={`flex items-center gap-3 px-6 py-4 rounded-xl shadow-2xl border ${toast.type === 'error' ? 'bg-red-50 border-red-200 text-red-800' : 'bg-emerald-50 border-emerald-200 text-emerald-800'}`}>
          {toast.type === 'error' ? <AlertCircle className="w-5 h-5" /> : <CheckCircle className="w-5 h-5" />}
          <p className="font-medium text-sm">{toast.message}</p>
          <button onClick={() => setToast({ show: false, message: "", type: "success" })} className="ml-4 opacity-60 hover:opacity-100"><X className="w-4 h-4" /></button>
        </div>
      </div>
        ...
        {pendingActions.length > 0 ? (
          <div className="bg-gradient-to-r from-indigo-900 to-purple-900 rounded-2xl p-6 shadow-lg text-white flex flex-col sm:flex-row justify-between items-center gap-6 animate-in slide-in-from-top-4">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-white/20 rounded-full animate-pulse"><BellRing className="w-8 h-8 text-white" /></div>
              <div>
                <h2 className="text-xl font-bold">Emails Due Today</h2>
                <p className="text-indigo-200 text-sm mt-1">You have <strong className="text-white">{pendingActions.length} strictly scheduled emails</strong> that need to be sent today.</p>
              </div>
            </div>
            <button 
              onClick={handleBulkEmail} disabled={processingBulk}
            ...
            >
              {processingBulk ? <Loader className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 flex items-center gap-4 text-emerald-800">
             <div className="p-3 bg-emerald-100 rounded-full"><CheckCircle className="w-6 h-6 text-emerald-600" /></div>
             <div>
               <h2 className="text-lg font-bold">You are all caught up!</h2>
               <p className="text-sm">There are no emails scheduled to be sent today. Check back tomorrow.</p>
             </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto">
            <div className="relative w-full sm:w-64 group">
              ...
              <input type="text" placeholder="Search Name or ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div className="relative w-full sm:w-40">
              <Filter className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" />
              <select value={weekFilter} onChange={(e) => setWeekFilter(e.target.value)} className="w-full pl-9 pr-8 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm appearance-none font-medium">
        ...
        {filteredStudentIds.length === 0 ? (
          ...
          <div className="space-y-6">
            {filteredStudentIds.map(studentId => {
              ...
              return (
                <div key={studentId} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                  <div onClick={() => toggleStudent(studentId)} className="bg-gradient-to-r from-slate-50 to-white px-6 py-5 border-b border-gray-100 flex justify-between items-center cursor-pointer group">
                    <div className="flex items-center gap-4">
                      <span className="w-10 h-10 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-black">{studentData.name.substring(0, 2).toUpperCase()}</span>
                      <div><h2 className="text-xl font-bold text-slate-800">{studentData.name}</h2><p className="text-sm text-gray-500">ID: {studentId}</p></div>
                    </div>
                    <button className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500">{isExpanded ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}</button>
                  </div>
                  ...
                  <div className={`transition-all duration-500 ease-in-out ${isExpanded ? 'max-h-[5000px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                    <div className="p-6 space-y-6">
                      {studentData.workloads.sort((a, b) => b.week - a.week).map(workload => {
                          const btnConfig = getEmailButtonConfig(workload);
                          const isProcessingThisEmail = processingId === `email-${studentId}-${workload.week}`;
                          const totalTasks = workload.breakdown?.length || 0;
                          const completedTasks = workload.breakdown?.filter(t => t.isCompleted).length || 0;
                          const progress = totalTasks === 0 ? 0 : Math.round((completedTasks / totalTasks) * 100);

                          return (
                            <div key={workload.id} className="border border-gray-100 rounded-xl overflow-hidden shadow-sm">
                              <div className="bg-white px-5 py-4 flex flex-col sm:flex-row justify-between items-center gap-4 border-b border-gray-100">
                                <div className="flex-1 w-full">
                                  <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <span className="text-lg font-bold">Week {workload.week}</span>
                                    <span className={`px-2.5 py-1 rounded-md text-xs font-bold uppercase ${workload.status === 'NORMAL' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{workload.status}</span>
                                  </div>
                                  <div className="flex items-center gap-3">
                                    <div className="w-48 h-2 bg-gray-100 rounded-full"><div className="h-full bg-indigo-500" style={{ width: `${progress}%` }} /></div>
                                    <span className="text-xs font-semibold text-gray-500">{completedTasks}/{totalTasks} Done</span>
                                  </div>
                                </div>
                                <button onClick={() => handleSendEmail(studentId, workload.week)} disabled={btnConfig.disabled || isProcessingThisEmail} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold border ${btnConfig.disabled ? 'cursor-not-allowed opacity-80' : 'active:scale-95'} ${btnConfig.colorClass}`}>
                                  {isProcessingThisEmail ? <Loader className="w-4 h-4 animate-spin" /> : <btnConfig.Icon className="w-4 h-4" />}
                                  {isProcessingThisEmail ? "Processing..." : btnConfig.text}
                                </button>
                              </div>
                              <div className="divide-y divide-gray-50 bg-slate-50/50">
                                {workload.breakdown?.map((task, index) => {
                                  const isCompleted = task.isCompleted || false;
                                  const isProcessingTask = processingId === `task-${workload.id}-${index}`;
                                  return (
                                    <div key={index} className="flex items-center justify-between p-4">
                                      <div className="flex items-center gap-4">
                                        <button onClick={() => handleToggleCompletion(workload.id, index, isCompleted)} disabled={isProcessingTask} className="active:scale-75 transition-transform">
                                          {isProcessingTask ? <Loader className="w-6 h-6 animate-spin text-indigo-400" /> : isCompleted ? <CheckCircle className="w-6 h-6 text-emerald-500" /> : <Circle className="w-6 h-6 text-gray-300" />}
                                        </button>
                                        <div>
                                          <p className={`font-semibold ${isCompleted ? 'text-gray-400 line-through' : 'text-gray-800'}`}>{task.subjectName}</p>
                                          <p className="text-xs text-gray-500">{task.type} • {task.hours} Hours</p>
                                        </div>
                                      </div>
                                      {!isCompleted && <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-md border border-amber-200"><AlertCircle className="w-3 h-3 inline mr-1" /> Pending</span>}
                                    </div>
                                  );
                                })}
                              </div>
