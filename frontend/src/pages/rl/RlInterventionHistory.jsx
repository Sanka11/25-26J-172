...
const COLLECTION = "student_rl_intervention_history";

function parseWeek(w) {
  const m = String(w || "").match(/(\d{4})-W(\d+)/);
  if (m) return parseInt(m[1]) * 100 + parseInt(m[2]);
  const n = parseInt(String(w || "").replace(/\D/g, ""));
  return n || 0;
}

const ACTION_CFG = {
  SOFT_NUDGE:       { label: "Soft Nudge",   badge: "bg-sky-50 text-sky-700 border-sky-200",          dot: "bg-sky-500"    },
  REMINDER:         { label: "Reminder",      badge: "bg-slate-100 text-slate-700 border-slate-300",   dot: "bg-slate-500"  },
  PEER_CHEER:       { label: "Peer Cheer",    badge: "bg-blue-50 text-blue-700 border-blue-200",       dot: "bg-blue-500"   },
  HUMAN_ESCALATION: { label: "Escalate",      badge: "bg-red-50 text-red-700 border-red-200",          dot: "bg-red-500"    },
  DO_NOTHING:       { label: "No Action",     badge: "bg-slate-50 text-slate-500 border-slate-200",    dot: "bg-slate-300"  },
};

const RISK_CFG = {
  HIGH:   { badge: "bg-red-50 text-red-700 border-red-200",             dot: "bg-red-500"     },
  NORMAL: { badge: "bg-amber-50 text-amber-700 border-amber-200",       dot: "bg-amber-500"   },
  LOW:    { badge: "bg-emerald-50 text-emerald-700 border-emerald-200", dot: "bg-emerald-500" },
};

function ActionBadge({ action }) {
  const c = ACTION_CFG[action] || { label: action || "—", badge: "bg-slate-50 text-slate-500 border-slate-200", dot: "bg-slate-300" };
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${c.badge}`}>
...
function RiskBadge({ level }) {
  const c = RISK_CFG[level] || { badge: "bg-slate-50 text-slate-500 border-slate-200", dot: "bg-slate-300" };
  return (
...
function TrendPill({ trend }) {
  const map = {
    INCREASING: { label: "Increasing", cls: "text-slate-600 border-slate-300 bg-slate-50", icon: "↑" },
    DECREASING: { label: "Decreasing", cls: "text-slate-600 border-slate-300 bg-slate-50", icon: "↓" },
    STABLE:     { label: "Stable",     cls: "text-slate-500 border-slate-200 bg-slate-50", icon: "→" },
  };
  const c = map[trend] || { label: trend || "—", cls: "text-slate-400 border-slate-200 bg-slate-50", icon: "—" };
  return (
...
function MiniBar({ value, max, colorClass }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="font-mono text-xs font-semibold text-slate-600 w-4 text-right tabular-nums">{value ?? 0}</span>
...
export default function RlInterventionHistory() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const result = await getRlHistory();
      setData(result);
    } catch (err) {
      console.error("Error loading RL history:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteAllForStudent = async () => {
    ...
    try {
      const snap = await getDocs(
        query(collection(db, COLLECTION), where("student_id", "==", confirmDeleteAll))
      );
  ...
  const studentMap = useMemo(() => {
    ...
    Object.values(map).forEach(arr => arr.sort((a, b) => parseWeek(b.week) - parseWeek(a.week)));
    return map;
  }, [data]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      records:     data.length,
      highRisk:    latest.filter(r => r.risk_level === "HIGH").length,
      escalations: latest.filter(r => r.action === "HUMAN_ESCALATION").length,
      avgFatigue:  latest.length
        ? (latest.reduce((s, r) => s + (r.fatigue_level || 0), 0) / latest.length).toFixed(1)
        : 0,
    };
  }, [students, data]);

  const maxStreak  = useMemo(() => Math.max(1, ...data.map(r => r.no_response_streak || 0)), [data]);
  const maxFatigue = useMemo(() => Math.max(1, ...data.map(r => r.fatigue_level || 0)), [data]);

  const filtered = useMemo(() =>
    students.filter(s => {
      if (search        && !s.student_id.toLowerCase().includes(search.toLowerCase())) return false;
      if (riskFilter   !== "all" && s.latest.risk_level !== riskFilter)   return false;
      if (actionFilter !== "all" && s.latest.action     !== actionFilter) return false;
      return true;
    })
  , [students, search, riskFilter, actionFilter]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-semibold text-slate-900 tracking-tight">RL Intervention History</h1>
              <p className="text-sm text-slate-500 mt-0.5">
                Adaptive intervention decisions · latest snapshot per student · click to expand
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            ...
            { label: "Escalations",        value: stats.escalations, accent: "border-orange-400",  num: "text-slate-800" },
            { label: "Avg Fatigue",        value: stats.avgFatigue,  accent: "border-slate-300",   num: "text-slate-800" },
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              ...
              <option value="PEER_CHEER">Peer Cheer</option>
              <option value="HUMAN_ESCALATION">Human Escalation</option>
              <option value="DO_NOTHING">No Action</option>
            </select>
            <button
              onClick={loadData}
              disabled={loading}
          ...
          {(search || riskFilter !== "all" || actionFilter !== "all") && (
            <div className="mt-3 flex flex-wrap gap-2 items-center">
              ...
              {actionFilter !== "all" && <Chip label={`Action: ${actionFilter}`} onRemove={() => setActionFilter("all")} />}
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {loading && (
            <div className="py-16 flex flex-col items-center gap-3">
              ...
              <p className="text-sm text-slate-500">Loading intervention history…</p>
            </div>
          )}
          ...
          {!loading && filtered.length === 0 && (
            <div className="py-16 flex flex-col items-center gap-3 text-center">
              ...
              <p className="text-xs text-slate-400">Try adjusting your filters or run a prediction first</p>
            </div>
          )}
                ...
                return (
                  <div key={student_id} className="group">
                    ...
                    <div className={`px-5 py-3.5 transition-colors ${isOpen ? "bg-blue-50/40" : "hover:bg-slate-50/80"}`}>
                      <div className="flex items-center gap-3">
                        ...
                        >
                          ...
                          <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-400 shrink-0">
                            <span>Fatigue</span>
                            <MiniBar value={latest.fatigue_level ?? 0} max={maxFatigue}
                              colorClass={latest.fatigue_level >= 3 ? "bg-red-400" : latest.fatigue_level >= 2 ? "bg-amber-400" : "bg-emerald-400"} />
                          </div>
                          <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-400 shrink-0">
                            <span>Streak</span>
                            <MiniBar value={latest.no_response_streak ?? 0} max={maxStreak}
                              colorClass={latest.no_response_streak >= 3 ? "bg-red-400" : latest.no_response_streak >= 2 ? "bg-amber-400" : "bg-slate-300"} />
                          </div>
                        </button>
                      ...
                      {latest.decision_reason && !isDeletingAll && (
                        <p className="mt-1.5 ml-[48px] text-xs text-slate-400 line-clamp-1">
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="bg-slate-50 border-b border-slate-200 text-left text-xs text-slate-500 uppercase tracking-wider">
                                ...
                                <th className="px-4 py-3 font-semibold">Prev Action</th>
                                <th className="px-4 py-3 font-semibold">Action Taken</th>
                                <th className="px-4 py-3 font-semibold">Fatigue</th>
                                ...
                                return (
                                  ...
                                  >
                                    <td className="px-4 py-3 whitespace-nowrap">
                                      <div className="flex items-center gap-2">
                                        <span className="font-mono text-xs text-slate-700">{rec.week}</span>
                                        {i === 0 && (
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap"><RiskBadge level={rec.risk_level} /></td>
                                    <td className="px-4 py-3 whitespace-nowrap"><TrendPill trend={rec.risk_trend} /></td>
                                    <td className="px-4 py-3 whitespace-nowrap">
                                      <span className="text-xs text-slate-400 font-mono">{rec.last_action || "—"}</span>
                                    </td>
                                    <td className="px-4 py-3 whitespace-nowrap"><ActionBadge action={rec.action} /></td>
                                    <td className="px-4 py-3 whitespace-nowrap">
                                      <MiniBar value={rec.fatigue_level ?? 0} max={maxFatigue}
                                        colorClass={rec.fatigue_level >= 3 ? "bg-red-400" : rec.fatigue_level >= 2 ? "bg-amber-400" : "bg-emerald-400"} />
                                    </td>
                                    <td className="px-4 py-3 whitespace-nowrap">
                                      <MiniBar value={rec.no_response_streak ?? 0} max={maxStreak}
                                        colorClass={rec.no_response_streak >= 3 ? "bg-red-400" : rec.no_response_streak >= 2 ? "bg-amber-400" : "bg-slate-300"} />
                                    </td>
                                    <td className="px-4 py-3 text-xs text-slate-500 max-w-xs">
                                      <span className="line-clamp-2" title={rec.decision_reason}>
                                        {rec.decision_reason || "—"}
                                      </span>
                                    </td>
