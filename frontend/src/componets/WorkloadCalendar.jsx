...
const MAX_WEEKS = 17; // Hardcoded semester limit
...
const WorkloadCalendar = ({
  ...
  // Helper to get the absolute mathematical week (can return < 1 or > 17)
  const getRawAcademicWeek = (date) => {
    if (!semesterStartDate || !date) return 1;
    const start = startOfWeek(new Date(semesterStartDate), { weekStartsOn: 1 });
    const target = startOfWeek(new Date(date), { weekStartsOn: 1 });
    start.setHours(0, 0, 0, 0);
    target.setHours(0, 0, 0, 0);
    const diffDays = Math.floor(
      (target.getTime() - start.getTime()) / (1000 * 60 * 60 * 24),
  ...
  // Clamped week calculator for display (always 1 to 17)
  const calculateAcademicWeek = (date) => {
    const rawWeek = getRawAcademicWeek(date);
    return Math.min(Math.max(1, rawWeek), MAX_WEEKS);
  };
  ...
  // --- BOUNDARY LOGIC ---
  const semStart = useMemo(
    () =>
      startOfWeek(new Date(semesterStartDate || new Date()), {
        weekStartsOn: 1,
      }),
    [semesterStartDate],
  );
  const semEnd = useMemo(() => addWeeks(semStart, MAX_WEEKS - 1), [semStart]); // Week 17 start

  const isPrevDisabled = useMemo(() => {
    if (view === "week") return weekNumber <= 1;
    return startOfMonth(currentWeekStart) <= startOfMonth(semStart);
  }, [view, weekNumber, currentWeekStart, semStart]);

  const isNextDisabled = useMemo(() => {
    if (view === "week") return weekNumber >= MAX_WEEKS;
    return startOfMonth(currentWeekStart) >= startOfMonth(semEnd);
  }, [view, weekNumber, currentWeekStart, semEnd]);

  const handlePrevious = () => {
    if (!onWeekChange || isPrevDisabled) return;
    const newDate =
      view === "week"
        ? subWeeks(new Date(currentWeekStart), 1)
        : subMonths(new Date(currentWeekStart), 1);
  ...
  const handleNext = () => {
    if (!onWeekChange || isNextDisabled) return;
    const newDate =
      view === "week"
        ? addWeeks(new Date(currentWeekStart), 1)
        : addMonths(new Date(currentWeekStart), 1);
  ...
  const handleJumpToToday = () => {
    if (!onWeekChange) return;
    let todayTarget = startOfWeek(new Date(), { weekStartsOn: 1 });

    // Clamp the "Today" button so it never forces the calendar outside bounds
    if (todayTarget < semStart) todayTarget = semStart;
    if (todayTarget > semEnd) todayTarget = semEnd;

    onWeekChange(todayTarget);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-1 sm:gap-2 bg-white/50 p-1.5 rounded-2xl border border-white shadow-[0_4px_12px_rgba(0,0,0,0.03)] backdrop-blur-sm self-start md:self-auto">
          <button
            ...
            className={`p-2 sm:p-3 rounded-xl transition-all ${isPrevDisabled ? "text-gray-300 cursor-not-allowed" : "text-gray-600 hover:bg-white hover:shadow-md active:scale-95"}`}
          >
            <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" />
          </button>
          <button
          ...
          <button
            ...
            className={`p-2 sm:p-3 rounded-xl transition-all ${isNextDisabled ? "text-gray-300 cursor-not-allowed" : "text-gray-600 hover:bg-white hover:shadow-md active:scale-95"}`}
          >
            <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5" />
          </button>
        </div>
      ...
      {view === "week" ? (
        ...
        <div className="bg-white/60 backdrop-blur-xl rounded-3xl border border-white shadow-sm overflow-hidden">
          ...
          <div className="grid grid-cols-7 gap-[1px] bg-gray-100">
            ...
            {monthDays.map((date, i) => {
              const rawWeek = getRawAcademicWeek(date);
              const isOutOfBounds = rawWeek < 1 || rawWeek > MAX_WEEKS;

              const dayWorkload = workloadByWeek.get(rawWeek);
              const isToday = isSameDay(date, new Date());
              const isPacked =
                dayWorkload && isPackedStatus(dayWorkload.status);
              const isStartOfWeek = getDay(date) === 1;

              // If the day is before the semester starts or after week 17, render it disabled
              if (isOutOfBounds) {
                return (
                  <div
                    key={i}
                    className="min-h-[80px] sm:min-h-[120px] p-1.5 sm:p-3 bg-gray-50/40 border-gray-100 opacity-60 pointer-events-none"
                  >
