...
const WorkloadCalendar = ({
  ...
  // Simplified colors - just red for packed, green for normal
  const getStatusColor = (status) => {
    // Check if it's a packed week (BUSY, HEAVY, OVERLOADED)
    const isPacked =
      status === "BUSY" || status === "HEAVY" || status === "OVERLOADED";

    if (isPacked) {
      return "bg-gradient-to-br from-red-50 to-red-100/50 border-red-200 text-red-800 hover:shadow-[0_8px_20px_rgb(239,68,68,0.2)] hover:border-red-300";
    } else {
      return "bg-gradient-to-br from-emerald-50 to-green-50/50 border-emerald-200 text-emerald-800 hover:shadow-[0_8px_20px_rgb(16,185,129,0.2)] hover:border-emerald-300";
    }
  };
    ...
    if (isPacked) {
      return (
        <div className="relative">
          <AlertTriangle className="w-5 h-5 text-red-500 drop-shadow-sm" />
  ...
  const getStatusLabel = (status = "NORMAL") => {
    ...
    if (isPacked) {
      return {
        text: "Packed",
        emoji: "📚",
        desc: "Busy week ahead - prepare!",
      };
    } else {
      return { text: "Easy", emoji: "😌", desc: "Light workload - relax!" };
    }
  };
    ...
    <>
      ...
      <div className="p-5 sm:p-8 bg-white/50 backdrop-blur-sm">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <button
                ...
                className="p-1.5 rounded-full hover:bg-indigo-100 text-indigo-500 transition-all"
              ...
              <div className="mb-4 p-3 bg-indigo-50 rounded-xl border border-indigo-100 animate-fade-in">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>📚 Packed Week</span>
                  </div>
                </div>
                <p className="text-xs text-gray-500 mt-2">
                  Numbers on cards show how many events/exams you have that week
                </p>
              </div>
            )}

            <p className="text-sm font-semibold text-gray-500 mt-2 flex items-center gap-2 flex-wrap">
              <span className="px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-md">
                Week {weekNumber}
              </span>
              <span>•</span>
            ...
            {currentWorkload && (
              <div className="flex items-center gap-4 mt-4 animate-scale-in delay-stagger-0 flex-wrap">
                <span
                  className={`px-3.5 py-1.5 rounded-lg text-xs font-bold shadow-sm border flex items-center gap-1.5 ${
                    getStatusColor(currentWorkload.status).split(" ")[0]
                  } ${getStatusColor(currentWorkload.status).split(" ")[1]}`}
                >
                  {getStatusLabel(currentWorkload.status).emoji}
                  {getStatusLabel(currentWorkload.status).text} Week
                </span>
                <span className="flex items-center gap-1.5 text-sm font-bold text-gray-600 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                  <Clock className="w-4 h-4 text-gray-400" />
                  {currentWorkload.totalHours || 0} Total Hours
                ...
                {currentWorkload.events &&
                  currentWorkload.events.length > 0 && (
                    <span className="flex items-center gap-1.5 text-sm font-bold text-red-600 bg-red-50 px-3 py-1.5 rounded-lg border border-red-100">
                      <Calendar className="w-4 h-4" />
                      {currentWorkload.events.length}{" "}
                      {currentWorkload.events.length === 1 ? "Event" : "Events"}
                    </span>
                  )}
              </div>
            )}
          ...
          {weekDaysData.map((day, index) => {
            ...
            return (
              <button
                ...
                className={`relative p-5 rounded-2xl border-2 transition-all duration-300 animate-scale-in delay-stagger-${index}
                  ${statusInfo}
                  ${hasWorkload ? "cursor-pointer hover:-translate-y-1 hover:shadow-xl" : "cursor-default opacity-70"}
                  ${day.isToday ? "ring-4 ring-indigo-500/20 border-indigo-500" : ""}
                `}
                disabled={!hasWorkload}
                title={
                  hasWorkload
                    ? `Click to see ${statusLabel?.desc}`
                ...
                <div className="text-center flex flex-col items-center h-full justify-between min-h-[140px]">
                  ...
                  {hasWorkload ? (
                    <div className="w-full mt-auto pt-3 flex flex-col items-center gap-1.5">
                      ...
                      <div className="text-xs font-extrabold uppercase tracking-wider opacity-90 flex items-center gap-1">
                      ...
                      {/* SHOW EVENT COUNT FROM BACKEND - TWO ICONS EXAMPLE */}
                      {day.weekData.events &&
                        day.weekData.events.length > 0 && (
                          <div className="absolute -top-2 -right-2 flex gap-0.5">
                            {/* Show up to 2 event icons */}
                            {day.weekData.events
                              .slice(0, 2)
                              .map((event, idx) => (
                                <div
                                  key={idx}
                                  className="w-6 h-6 bg-red-500 text-white text-xs rounded-full flex items-center justify-center font-bold shadow-md ring-2 ring-white"
                                  title={event}
                                >
                                  {event.includes("Exam")
                                    ? "📝"
                            ...
                            {/* Show + if more than 2 events */}
                            {day.weekData.events.length > 2 && (
                              <div className="w-6 h-6 bg-gray-700 text-white text-xs rounded-full flex items-center justify-center font-bold shadow-md ring-2 ring-white">
                                +{day.weekData.events.length - 2}
                              </div>
                            )}
                          </div>
                      ...
                      {/* Floating Tooltip with event details */}
                      {isHovered && day.weekData.events?.length > 0 && (
                        <div className="absolute z-20 bottom-[105%] left-1/2 transform -translate-x-1/2 mb-2 px-4 py-2 bg-gray-900/95 backdrop-blur-md text-white text-xs rounded-xl shadow-xl whitespace-nowrap animate-scale-in pointer-events-none border border-gray-700">
                          <div className="flex items-center gap-2 font-semibold mb-1">
                            <Calendar className="w-3.5 h-3.5 text-red-400" />
                            {day.weekData.events.length} Event
                            {day.weekData.events.length !== 1 ? "s" : ""}:
                          </div>
                          {day.weekData.events.map((event, idx) => (
                            <div
                              key={idx}
                              className="text-left text-gray-200 text-xs"
                            >
                              • {event}
                            </div>
                          ))}
        ...
        <div className="mt-10 pt-8 border-t border-gray-100 flex flex-col xl:flex-row gap-8 justify-between">
          ...
          <div className="flex-1 bg-gradient-to-br from-indigo-50 to-blue-50/50 p-5 rounded-2xl border border-indigo-100">
            <h4 className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-3 flex items-center gap-2">
              <Info className="w-4 h-4" /> How to Use This Calendar
            </h4>
            <ul className="space-y-2.5 text-sm font-medium text-gray-600">
              <li className="flex items-start gap-2.5">
                <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 mt-1.5 flex-shrink-0"></div>
                <span>
                  <strong className="text-indigo-700">🎨 Colors:</strong> 🔴 Red
                  = Packed week ahead, 🟢 Green = Easy week
                </span>
              </li>
              <li className="flex items-start gap-2.5">
                <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 mt-1.5 flex-shrink-0"></div>
                <span>
                  <strong className="text-indigo-700">📌 Event Icons:</strong>{" "}
                  📝 = Exam, 📤 = Submission, 📌 = Other event
                  <span className="block text-xs text-gray-500 mt-1">
                    Hover over icons to see event details!
                  </span>
                </span>
              </li>
              <li className="flex items-start gap-2.5">
                <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 mt-1.5 flex-shrink-0"></div>
                <span>
                  <strong className="text-indigo-700">👆 Click any day</strong>{" "}
                  to see detailed breakdown of tasks
                </span>
              </li>
            </ul>
          </div>
