...
const RecommendationDashboard = () => {
  ...
  const [checkInData, setCheckInData] = useState({
    study_hours_per_week: 10,
    stress_level: 5, // Updated default to match the new "Manageable" card
    sleep_hours: 6.5,
  });
  ...
  useEffect(() => {
    ...
    const loadStudentData = async () => {
      try {
        ...
        // Fetch history using the exact student_id (myStudentId)
        const historyRef = collection(db, `students/${myStudentId}/history`);
        const q = query(historyRef, orderBy("timestamp", "desc"));
        const historySnap = await getDocs(q);
        ...
        // Fetch latest recommendations side-panel using student_id
        await fetchLatestRecommendations(myStudentId);
      } catch (error) {
        console.error("Error fetching Firestore data:", error);
      }
  ...
  const fetchAIInsights = async () => {
    ...
    try {
      ...
      // Use the readable student_id from academicData
      if (academicData && academicData.student_id) {
        await fetchLatestRecommendations(academicData.student_id);
  ...
  const toggleCheckbox = async (index) => {
    ...
    // Update task progress in the readable student_id folder
    if (academicData && academicData.student_id && currentDocId) {
      try {
        const docRef = doc(
          db,
                  ...
                  <div className="group">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {[
                        ...
                        { level: 5, emoji: "😐", label: "Manageable" },
                        { level: 8, emoji: "😟", label: "Stressed" },
                        { level: 10, emoji: "😫", label: "Overwhelmed" },
                      ].map((option) => (
                        <button
                          key={option.level}
                          onClick={() =>
                            setCheckInData((prev) => ({
                              ...prev,
                              stress_level: option.level,
                            }))
                          }
                          type="button"
                          className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all duration-200 ${
                            checkInData.stress_level === option.level
